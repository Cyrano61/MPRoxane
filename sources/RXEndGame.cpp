	...
	*/
	...
	for (int parity = 1; parity >= 0; parity--) {
		for(RXSquareList* empties = board.empties_list->next; empties->position != NOMOVE; empties = empties->next) {	
            ...
            if (board.local_Parity(empties->position) == parity && (legal_movesBB & 1ULL<<empties->position)) { ((board).*(board.generate_flips[empties->position]))(move);
				board.n_nodes++;
		...
		if(lower < upper) {
			...
			for (int parity = 1; lower < upper && parity >= 0; parity--) {
				for(RXSquareList* empties = board.empties_list->next; lower < upper && empties->position != NOMOVE; empties = empties->next) {
                    if (empties->position != hashmove && board.local_Parity(empties->position) == parity && (legal_movesBB & 1ULL<<empties->position)) { ((board).*(board.generate_flips[empties->position]))(move);
						board.n_nodes++;
...
//						*log << "Endgame: Only 1 move at n_empties: " << board.n_empties << " /selectivity : " << CONFIDENCE[selectivity] << "%" << std::endl;
	...
	hTable->update(   board.hashcode(), pv, type_hashtable, selective_cutoff? selectivity : NO_SELECT, DEPTH_BOOSTER+board.n_empties, alpha, beta, bestscore, bestmove);
	hTable_PV->update(board.hashcode(), pv, type_hashtable, selective_cutoff? selectivity : NO_SELECT, DEPTH_BOOSTER+board.n_empties, alpha, beta, bestscore, bestmove);
...
//				std::cout << "Error" << std::endl;
	...
	hTable->update(board.hashcode(), type_hashtable, selective_cutoff? selectivity : NO_SELECT, DEPTH_BOOSTER+board.n_empties, alpha, bestscore, bestmove);
	if(pvDev < 3)
		hTable_PV->update(board.hashcode(), type_hashtable, selective_cutoff? selectivity : NO_SELECT, DEPTH_BOOSTER+board.n_empties, alpha, bestscore, bestmove);
...
void RXEngine::check_PV(RXBBPatterns& sBoard, const int score, const int alpha, const int beta) {
	...
	if(board.n_empties>6) { //hash limit
		...
		if (movePV == NOMOVE) {
			...
			if(movePV != NOMOVE)
				hTable->update(board.hashcode(), true, type_hashtable, NO_SELECT, DEPTH_BOOSTER+board.n_empties, alpha, beta, value, movePV);
